...
export default function AboutPageClient() {
  ...
  return <main className="min-h-screen bg-white"><Breadcrumbs items={[{ name: t.nav.about }]} /><div className="container mx-auto px-4 md:px-12 lg:px-24 pt-4 md:pt-6 pb-2"><h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 mb-2 leading-snug">{text.title}</h1><div className="w-16 md:w-24 h-1 bg-[#174978]" /></div><div className="container mx-auto px-4 md:px-12 lg:px-24 py-4 md:py-6"><h2 className="text-base sm:text-lg md:text-xl font-bold text-[#003366] mb-4 leading-snug">{text.heading}</h2><div className="space-y-4 text-gray-700 text-sm md:text-base leading-relaxed">{text.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">{text.values.map((value) => <div key={value} className="bg-slate-50 p-5 text-center text-[#003366] font-bold">{value}</div>)}</div></div><Footer /><FloatingContacts /></main>;
}
